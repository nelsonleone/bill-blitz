...
<DatePicker.Root weekdayFormat="short" {value} onValueChange={handleDateChange} fixedWeeks={true}>
  <div class="flex flex-col gap-1.5 justify-end w-56">
    <DatePicker.Label class="block select-none font-overpass">Date</DatePicker.Label>
    <DatePicker.Input
      let:segments
    ...
    {#if isOpen} <!-- Show DatePicker.Content only if isOpen is true -->
    <DatePicker.Content
      sideOffset={6}
      transition={fly}
              ...
              <DatePicker.GridHead>
                <DatePicker.GridRow class="mb-1 flex w-full justify-between">
                  {#each weekdays as day}
                    <DatePicker.HeadCell class="w-10 rounded-md text-xs !font-normal text-muted-foreground">
                  ...
                  <DatePicker.GridRow class="flex w-full">
                    {#each weekDates as date}
                      <DatePicker.Cell {date} class="relative size-10 !p-0 text-center text-sm">
                        ...
                        >
                          <div class="absolute top-[5px] hidden size-1 rounded-full bg-foreground transition-all group-data-[today]:block group-data-[selected]:bg-background" />
                          {date.day}
                        </DatePicker.Day>
                      </DatePicker.Cell>
                    {/each}
