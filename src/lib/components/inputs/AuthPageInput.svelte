...
<div class={`my-4 w-full ${containerStyles}`}>
  ...
  <div class="relative h-14 w-full">
    ...
    {#if inputType === "password"}
      <IconButton ariaControls={id} on:click={() => localInputType = localInputType === "password" ? "text" : "password" } styles="w-auto bg-transparent p-0 inline-block text-primary-dark-blue absolute top-0 mx-0 bottom-0 my-auto text-2xl z-10 right-0" ariaLabel={showPassword ? "hide password" : "hide password"}>
  ...
  {#if error}
    <p role="alert" class="font-rubik text-sm text-primary-accent-color3 mt-2 block">{error}</p>
  {/if}
</div>
